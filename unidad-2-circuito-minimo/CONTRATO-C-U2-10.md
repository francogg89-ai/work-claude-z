# CONTRATO PREVIO DE VERIFICACIÓN — C-U2-10
...
La propuesta define una verificación discriminante para C-U2-10, no autorizada aún. Se repiten el gate ambiental y P0 bajo identidades, autorizaciones y evidencia nuevas. Aunque el gate/P0 de C-U2-9 fueron acreditados históricamente, no se reutilizan como habilitación operativa: el gate puede quedar obsoleto y P0 debe verificar esta versión exacta del contrato/checkpoint. La repetición requiere congelamiento auditor y autorizaciones humanas nuevas.
...
Una vez aprobada la preparación ambiental y solo con el ejecutable explícito, se ejecuta la secuencia propia de C-U2-10 sin alterar orden, cardinalidad, estímulos, acciones humanas, controles, criterios ni reglas de detención. Todos los comandos Python se invocan como:

```text
.\.venv\Scripts\python.exe -m ...
```

La secuencia material, los mensajes M1, M1b, M2, M5 y M6, los doce bloques, R0, RZ, el baseline de M6, la evidencia, los secretos, los controles y los criterios C2.6, C2.7, C2.8, C2.10 y C2.11 están transcriptos íntegramente abajo, con estas sustituciones obligatorias: `C-U2-10`, `evidencia-c-u2-10`, marcadores `preparacion-c-u2-10` y `fin-c-u2-10`, y el ejecutable explícito indicado arriba.
...
Toda evidencia nueva pertenece exclusivamente a `unidad-2-circuito-minimo/evidencia-c-u2-10/`. La preparación ambiental queda separada de la evidencia discriminante. No se modifica ninguna evidencia histórica. No se instalan dependencias, no se abre Quick Tunnel, no se usa ChatGPT real y no se ejecuta C-U2-10 hasta una H-2 y autorización humana nuevas, ligadas al congelamiento exacto.
...
| C0 | una persona demuestra positivamente que ninguna app del circuito está instalada/conectada, mediante matriz por app y señales inequívocas de no instalación/no conexión; visibilidad en «Created by me» no es fallo |

### C0 — estado instalado/conectado, no visibilidad

C0 pasa solo si evidencia positiva demuestra que ninguna app/complemento del circuito está efectivamente instalada o conectada. La mera visibilidad en «Created by me» no es fallo; la mera ausencia de una definición tampoco demuestra el estado.

La persona inspecciona las definiciones visibles del circuito y registra cada app en `evidencia-c-u2-10/c0-matriz-estados.md`, con nombre exacto tal como aparece, ubicación, señal exacta de UI, interpretación, artefacto de evidencia y resultado. Deben figurar como mínimo:

| Nombre exacto | Señal probatoria que debe documentarse | Interpretación permitida |
|---|---|---|
| `Sonda propuestas U1` | `+` que inequívocamente ofrece agregar/conectar, o texto `Instalar complemento` | no instalada/no conectada |
| `Sonda propuestas U1 v2` | `+` que inequívocamente ofrece agregar/conectar, o texto `Instalar complemento` | no instalada/no conectada |
| `Circuito propuestas C-U2-5` | `+` que inequívocamente ofrece agregar/conectar, o texto `Instalar complemento` | no instalada/no conectada |
| cada otra app que pertenezca inequívocamente al circuito y siga visible | señal explícita de UI con el mismo estándar | no instalada/no conectada |

Solo se acepta `+` cuando el contexto/tooltip/acción visible demuestra inequívocamente que agrega o conecta; solo se acepta `Instalar complemento` cuando la acción se refiere a esa definición exacta. No inferir desconexión por ausencia de botones. Si el estado es desconocido, ambiguo o no se puede documentar por app, detenerse y solicitar intervención humana.
...
| P8 | `.\\.venv\\Scripts\\python.exe -m circuit.launch marcar preparacion-c-u2-10` |
...
propia llamada «Circuito propuestas C-U2-10» cuyo servidor MCP sea `<BASE>/mcp`, sin código ni token
...
Se ejecuta una vez, después de R4-2: `.\\.venv\\Scripts\\python.exe -m circuit.launch marcar fin-c-u2-10`;
...
En `unidad-2-circuito-minimo/evidencia-c-u2-10/`, con la redacción de secretos por valor. Un
...
| Caso | Se cumple si |
|---|---|
| verificación previa | P0 termina con código 0, sin pruebas fallidas ni omitidas, y su salida es anterior a P1; C0 aporta captura y matriz completa por app con señal inequívoca de no instalada/no conectada; no queda ninguna app del circuito instalada/conectada |
| preparación | P2, P3, P7 y P8 terminan sin error; cada comando de P2, P3, P4, P7, P8 y P10 figura en su salida exactamente con la forma literal de la secuencia; P7 imprime exactamente `P-001` a `P-008`; en la exportación `calibraciones.convocatoria-1.reviewed_at` existe, es anterior al `received_at` de `P-001` y su interpretación dice «ejecutor local»; `P-001` a `P-008` pertenecen a `convocatoria-1` y son anteriores al marcador `preparacion-c-u2-10` |
...
- C0 no acredita positivamente el estado no instalado/no conectado de cada definición del circuito visible; hay una app instalada/conectada; falta una app en la matriz; la señal no es inequívoca; o se ejecuta desinstalación/desconexión sin autorización humana específica previa;
...
- El checkpoint sucesor, `CHECKPOINT_HUMANO-C-U2-10.md`, se entrega en esta misma intervención y
  vale solo si el AUDITOR congela este contrato sin cambios.

## Verificación de esta propuesta documental

Se comprobó la identidad base indicada, se leyeron el contrato/checkpoint C-U2-9 y las dos fuentes auditoras fijadas arriba. Esta propuesta solo crea los dos documentos sucesores. No se ejecutó gate, P0, C0 ni ninguna operación de UI; no se materializó evidencia operacional ni se modificó el entorno. La revisión de consistencia documental no equivale a congelamiento ni aprobación del AUDITOR.
...
## Garantías adicionales de C-U2-10

C-U2-10 es un sucesor separado de C-U2-9 conforme a REVOLUTIONS §6.1. C-U2-9 permanece cerrado/bloqueado por la ruptura de autoridad y el criterio de C0; no se reabre ni se reintenta. Su gate y P0 acreditados quedan como evidencia histórica, no como autorización ni evidencia operativa reutilizable en C-U2-10.
