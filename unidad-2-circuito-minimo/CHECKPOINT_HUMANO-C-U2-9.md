# CHECKPOINT HUMANO — C-U2-9

## Estado

Este checkpoint es una propuesta sucesora de C-U2-8. No autoriza ejecución, instalación ni modificación del entorno. Solo será válido si el AUDITOR congela exactamente este checkpoint y el contrato C-U2-9, y si luego existe una H-2 nueva y una autorización humana nueva para esas identidades.
...
3. El CONSTRUCTOR agrega únicamente `evidencia-c-u2-9/autorizaciones/gate-intento-N.md` sobre `PARENT_WORK_SHA` y devuelve `WORK_SHA_AUTORIZACION` y `AUTH_BLOB_SHA`.
...
El gate es PREVIO y NO DISCRIMINANTE. Si falla, se preserva únicamente `evidencia-c-u2-9/gate-intento-<N>/comandos.txt` y, si corresponde, su `DETENCION.md` en el mismo directorio; se emite `human_need` y C-U2-9 queda CONGELADO/NO_EJECUTADO/NO_AGOTADO. No se instala ni modifica nada sin autorización humana separada. Tras una remediación autorizada, solo puede repetirse con autorización nueva y explícita del AUDITOR y del humano. Un gate exitoso habilita P0 una sola vez.
...
La secuencia posterior completa está transcripta en este checkpoint: C0, P1-P11, R5/R6, R3 antes de R0, R0, R1/R2/R4 y RZ, con doce conversaciones, los mismos estímulos y las mismas acciones humanas. No se reordenan, omiten, repiten ni adaptan pasos. Solo cambian el identificador `C-U2-9`, el directorio `evidencia-c-u2-9`, los marcadores sucesores y la forma del ejecutable Python.
...
| P8 | consola | `.\\.venv\\Scripts\\python.exe -m circuit.launch marcar preparacion-c-u2-9` |
...
`R1-1`, `R1-2`, `R2-1`, `R2-2`, `R4-1`, `R4-2`. RZ empieza con `fin-c-u2-9`. Los marcadores
...
2. Una persona crea en ChatGPT web, modo desarrollador, una app propia llamada «Circuito propuestas
   C-U2-9» cuyo servidor MCP sea `<BASE>/mcp`. **La URL no lleva ningún código ni token.**
...
1. `.\\.venv\\Scripts\\python.exe -m circuit.launch marcar fin-c-u2-9`;
...
`unidad-2-circuito-minimo/evidencia-c-u2-9/`. Cada salida de consola empieza con el comando
...
Este checkpoint no autoriza ejecución por sí solo. Antes de cada gate, la autorización humana material para ese intento debe estar ya comprometida en `evidencia-c-u2-9/autorizaciones/gate-intento-<N>.md` y su blob exacto debe estar referenciado por el AUDITOR. Cada intento usa una ruta nueva e inmutable `evidencia-c-u2-9/gate-intento-<N>/`; está prohibido sobrescribir o modificar evidencia de cualquier intento anterior. Una autorización posterior al comando no regulariza el intento.

## Handshake previo obligatorio

La autorización humana se registra primero ante el AUDITOR y debe referir el `BASE_WORK_SHA` existente, el número de intento y las identidades del contrato y checkpoint. El CONSTRUCTOR sólo materializa luego el archivo `evidencia-c-u2-9/autorizaciones/gate-intento-<N>.md` sin anticipar el SHA de ese propio commit. El AUDITOR verifica el blob y congela el `WORK_SHA` resultante antes de permitir cualquier comando del gate. Una autorización posterior al gate no valida nada.

El gate se documenta únicamente en `evidencia-c-u2-9/gate-intento-<N>/comandos.txt`; si falla, la detención queda en ese mismo directorio.

## Namespace limpio y orden de autorización

C-U2-8 queda bloqueado y no ejecutado; este checkpoint no lo reabre. Toda evidencia de C-U2-9 usa exclusivamente `evidencia-c-u2-9/`, con un primer `gate-intento-1/` nuevo. No se modifica ni se consume la autorización histórica de C-U2-8.

Antes de materializar la autorización debe existir el sobre auditor que permite únicamente MATERIALIZAR. El CONSTRUCTOR materializa sólo `evidencia-c-u2-9/autorizaciones/gate-intento-1.md`, devuelve `WORK_SHA_AUTORIZACION` y `AUTH_BLOB_SHA`, y se detiene. Sólo después de la verificación del AUDITOR y de un nuevo sobre `EJECUCION_GATE` puede comenzar el gate. La autorización humana y el congelamiento del contrato no sustituyen el sobre de materialización.