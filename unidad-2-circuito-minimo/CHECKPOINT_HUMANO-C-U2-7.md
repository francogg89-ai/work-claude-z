...
El gate es PREVIO y NO DISCRIMINANTE. Si falla, se preserva `entorno-preparacion.txt`, se emite `human_need` y C-U2-7 queda CONGELADO/NO_EJECUTADO/NO_AGOTADO. No se instala ni modifica nada sin autorización humana separada. Tras una remediación autorizada, solo puede repetirse con autorización nueva y explícita del AUDITOR y del humano. Un gate exitoso habilita P0 una sola vez.
...
| P8 | consola | `.\\.venv\\Scripts\\python.exe -m circuit.launch marcar preparacion-c-u2-7` |
...
`R1-1`, `R1-2`, `R2-1`, `R2-2`, `R4-1`, `R4-2`. RZ empieza con `fin-c-u2-7`. Los marcadores
...
2. Una persona crea en ChatGPT web, modo desarrollador, una app propia llamada «Circuito propuestas
   C-U2-7» cuyo servidor MCP sea `<BASE>/mcp`. **La URL no lleva ningún código ni token.**
...
1. `.\\.venv\\Scripts\\python.exe -m circuit.launch marcar fin-c-u2-7`;
...
`unidad-2-circuito-minimo/evidencia-c-u2-7/`. Cada salida de consola empieza con el comando
