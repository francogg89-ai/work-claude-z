...
La secuencia posterior completa está transcripta en este checkpoint. C0 tiene el criterio de estado actualizado y no autoriza remover nada; P1-P11, R5/R6, R3 antes de R0, R0, R1/R2/R4 y RZ conservan su secuencia, doce conversaciones, estímulos y acciones definidos aquí. No se reordenan, omiten, repiten ni adaptan pasos. Solo cambian el identificador `C-U2-10`, el directorio `evidencia-c-u2-10`, los marcadores sucesores y la forma del ejecutable Python.
...
- Nunca modificar evidencia histórica de C-U2-4, C-U2-5, C-U2-6 o C-U2-9.
...
## C0 — verificar estado instalado/conectado

C0 evalúa el estado, no la visibilidad absoluta. Las definiciones históricas pueden seguir en «Created by me» si la evidencia demuestra que no están instaladas/conectadas.
...
| P8 | consola | `.\\.venv\\Scripts\\python.exe -m circuit.launch marcar preparacion-c-u2-10` |
...
`R1-1`, `R1-2`, `R2-1`, `R2-2`, `R4-1`, `R4-2`. RZ empieza con `fin-c-u2-10`. Los marcadores
...
2. Una persona crea en ChatGPT web, modo desarrollador, una app propia llamada «Circuito propuestas
   C-U2-10» cuyo servidor MCP sea `<BASE>/mcp`. **La URL no lleva ningún código ni token.**
...
1. `.\\.venv\\Scripts\\python.exe -m circuit.launch marcar fin-c-u2-10`;
...
`unidad-2-circuito-minimo/evidencia-c-u2-10/`. Cada salida de consola empieza con el comando
...
| Caso | Se cumple si |
|---|---|
| verificación previa | P0 termina con código 0, sin pruebas fallidas ni omitidas, y su salida es anterior a P1; C0 tiene captura(s) y matriz completa que prueban positivamente estado no instalado/no conectado para cada app del circuito visible |
...
- C0 carece de prueba positiva inequívoca por app, hay app instalada/conectada, aparece estado desconocido, falta una fila/captura, o se desinstala/desconecta sin autorización humana específica previa;
